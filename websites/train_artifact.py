...
def parse_arguments():
	...
	parser.add_argument(
		...
		type=str,
		default="full",
		help="subset or subclass stratified by bias or factuality",
	)
...
if __name__ == "__main__":
	...
	out_dir = os.path.join(args.home_dir, "data", args.dataset,
		f"results", f"no_wiki_{args.task}_news")
	    #f"results", f"{args.task}_{','.join(args.features)}")
	...
	# identify correct dataset and splits from --subset arg
	
	data_sb = "corpus.tsv"
	splitset = "splits.json"
	
	if args.subset == "low":
		data_sb = "corpus_low.tsv"
		splitset = "splits_low.json"
	elif args.subset == "mixed":
		data_sb = "corpus_mixed.tsv"
		splitset = "splits_mixed.json"
	elif args.subset == "high":
		data_sb = "corpus_high.tsv"
		splitset = "splits_high.json"
	elif args.subset == "left":
		data_sb = "corpus_left.tsv"
		splitset = "splits_left.json"
	elif args.subset == "center":
		data_sb = "corpus_center.tsv"
		splitset = "splits_center.json"
	elif args.subset == "right":
		data_sb = "corpus_right.tsv"
		splitset = "splits_right.json"
	elif args.subset == "small":
		data_sb = "corpus_small.tsv"
		splitset = "splits_small.json"
	elif args.subset == "med":
		data_sb = "corpus_med.tsv"
		splitset = "splits_med.json"


	# read the dataset
	df = pd.read_csv(os.path.join(args.home_dir, "data", data_sb), sep="\t")
	...
	splits = json.load(open(os.path.join(args.home_dir, "splits", splitset), "r"))
	...
	features = {feature: json.load(open(os.path.join(args.home_dir, "features", f"{feature}.json"), "r")) for feature in args.features}
...
#		X["train"] = np.asmatrix([list(itertools.chain(*[features[feat][url] for feat in args.features])) for url in urls["train"]]).astype("float")
		X["train"] = np.array([list(itertools.chain(*[features[feat][url] for feat in args.features])) for url in urls["train"]]).astype("float")
...
#		X["test"] = np.asmatrix([list(itertools.chain(*[features[feat][url] for feat in args.features])) for url in urls["test"]]).astype("float")
		X["test"] = np.array([list(itertools.chain(*[features[feat][url] for feat in args.features])) for url in urls["test"]]).astype("float")
